...
def inference_unit_deploy(request):
    ...
    if(not check_service(data['position_uid'])):
        ...
        add_kong(
            position_route_host=f"{customized_env.HOST_IP}:{customized_env.ROUTE_MGT_PORT}",
            payload=payload
        )
        
    #deploy slave pod
    count = deploy_inference_host(data['application_uid'], data['position_uid'], data['num_of_deployment'], data['model_uid'], data['file_extension'])
...
def inference_unit_remove(request):
    ...
    if(data['num_of_deployment'] == '0'):
        ...
        count = delete_inference_host(data['position_uid'], data['num_of_deployment'])
    ...
    response = json.dumps({'status' : 'success', 'actual_num_of_removing' : count})
...
def inference_unit_update(request):
    ...
    response = json.dumps({'status' : 'success', 'model_uid': data['model_uid']})
